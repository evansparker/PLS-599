# this is the modified file needed to use fasta match regex to search for sequence motifs
...
use Exporter;
our @ISA = qw/Exporter/;
our @EXPORT = 'new copy fastas2string string2fastas is_same_length strip_chars canonical uc lc composition seq_lengths seq_lengths_detail is_defline_good is_record is_records match _is_comment _is_sequence ';
...
#    my $s_ = shift; # $_ [0]
    ...
    if ($_ [0] eq '') { return \@array; }

    my @records = split ("\n>", $_ [0]);    
    
    if ($records [0] =~ /^\s*$/) { shift (@records); }
    elsif (substr ($records [0], 0, 1) ne '>') 
    { 
        die 'The FASTA file does not start with \'>\'' . "\n" . 'Died'; 
    }
    else { $records [0] = substr ($records [0], 1); }
	
    foreach my $record (@records) 
    {
        $record = '>' . $record;
        push (@array, new Jls::Fasta ($record));
    }
...
{   
    ...
    foreach my $fasta (@$fastas_)
    {
        ...
        eval
        {
            ...
            if ($is_case_insensitive_ && $fasta->{'seq'} =~ m=$regex_=i) 
                { $push = 1; }
            elsif (! $is_case_insensitive_ && $fasta->{'seq'} =~ m=$regex_=) 
                { $push = 1; }
            else { $push = 0; }
        };
        ...
        if ($@) 
        {
            ...
            if ($is_case_insensitive_) { $str .= 'in'; }
            $str .= 'sensitive match';
            $str .= ' could not be applied to the sequence' . "\n";
            $str .= $fasta->{'seq'};
